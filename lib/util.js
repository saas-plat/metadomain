...
const parseBigData = (data) => {
  return _.mapValues(data, (it) => {
    // $开头系统字段
    if (it && (it.$type === 'BigArray' || (Array.isArray(it.$fields) && Array.isArray(it.$data)))) {
      const {
        fields = [], data = []
      } = it;
      const _fields = it.$fields || fields;
      const _data = it.$data || data;
      const bigarr = _data.map(row => {
      ...
      return it.map(sit => parseBigData(sit));
    } else if (_.isPlainObject(it)) {
      return parseBigData(it);
    } else {
      return it;
    }
  });
    ...
    if (it.type === 'object' || it.subtype === 'object') {
      ...
      if (nKey) {
        ...
        if (!_.isUndefined(pFields[nKey])) {
          debug('conflict %s', nKey, pFields[nKey], it[nKey]);
          errFields.push(it);
        } else {
          defectFields.push({
            ...it,
