...
const types = exports.types = ['string', 'object', 'array', 'number', 'boolean', 'date', 'reference'];
const getMeta = exports.getMeta = (obj) => {
  const meta = [];
  obj && Object.keys(obj).forEach(key => {
    ...
    if (typeof obj[key] === 'string') {
      ...
      obj[key].type = obj[key].type || obj[key].Type;
      //debug(obj[key].type,obj[key].fields)
      type = obj[key].type === 'ref' ? 'reference' : obj[key].type;
      fields = type === 'obj' || type === 'object' || type === 'reference' || type === 'array' ?
        getMeta(obj[key].fields || obj[key].Fields) :
        undefined;
      defValue = obj[key].defValue || obj[key].DefValue;
    } else if (Array.isArray(obj[key])) {
      type = 'array';
      if (obj[key].length > 0) {
        const arrobj = obj[key][0];
        if (arrobj.type) {
          fields = getMeta(arrobj.fields);
        } else {
          fields = getMeta(arrobj);
        }
      }
    } else if (typeof obj[key] === 'object') {
      type = 'reference'; // 默认是引用类型
...
exports.noenumerable = function noenumerable(target, key) {
  Object.defineProperty(target, key, {
    enumerable: false
  });
}

exports.readonly = function readonly(target, key, initValue) {
  Object.defineProperty(target, key, {
    writable: true
  });
...
exports.readonly2 = function readonly(target, key ) {
  Object.defineProperty(target, key, {
    writable: false,
  });
}
