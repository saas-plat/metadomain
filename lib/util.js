...
const getMeta = exports.getMeta = (obj, types) => {
  const meta = [];
  _.isPlainObject(obj) && Object.keys(obj).forEach(key => {
    let type;
    let fields;
    let defValue;
    let src;
    ...
    if (typeof obj[key] === 'string') {
      ...
      fields = type === 'obj' || type === 'object' || type === 'array' ?
        getMeta(fields, types) :
        undefined;
      defValue = other['default'] || other.defValue || defaultValue;
      src = src;
      rules = {
        type,
        ...other
      ...
      if (obj[key].length > 0) {
        ...
        if (_.isPlainObject(arrobj)){
          subtype = arrobj.type === 'ref' ? 'reference' : arrobj.type;
          subtype = arrobj.type === 'obj' ? 'object' : arrobj.type;
        }
...
const checkRequiredFieldMap = exports.checkRequiredFieldMap = (fields, mappings) => {
  const errFields = [];
  const defectFields = [];
  fields.forEach(it => {
    ...
    if (it.type === 'reference') {
      ...
      const rfm = checkRequiredFieldMap(it.fields, mappings[it.key]);
      defectFields.push({
        ...it,
...
const diff = exports.diff = (target, compare) => {
  ...
  if (Array.isArray(target)) {
    ...
    compare = compare || {};
    // 新增的字段
    let right = _.omitBy(target, (val, key) => !(key in compare));
    //debug(11,right)
    right = _.mapValues(right, (val, key) => diff(val, undefined));
    //debug(11,right)
    right = _.omitBy(right, (val) => _.isUndefined(val));
    // 不等的字段，但是undefined不算不等
    // _type等下划线开头的字段是entity自动添加的系统字段需要排除
    let v = _.omitBy(target, (val, key) => _.isFunction(val) || !(key in compare) || _.isUndefined(val) ||
      _.isEqual(val, compare[key]) || key.startsWith('_'));
    v = _.mapValues(v, (val, key) => diff(val, compare[key]));
    // 排除undefined对象
    v = _.omitBy(v, (val) => _.isUndefined(val));
    // 如果是引用类型需要保留id否则无法更新
    const refid = {};
    if (target.id) {
      refid.id = target.id;
