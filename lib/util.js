...
const getMetaItem = (it, types) => {
  ...
  let mapping;
  if (typeof it === 'string') {
    type = types.indexOf(it.toLowerCase()) > -1 ?
      it.toLowerCase() :
      'reference';
    if (type === 'reference' && it !== 'reference') {
      src = it;
    }
  } else if ('type' in it) {
    let {
    ...
    defValue = it['default'] || it.defValue || defaultValue;
    src = it.src;
...
const checkRequiredFieldMap = exports.checkRequiredFieldMap = (fields, mappings) => {
  ...
  fields.forEach(it => {
    ...
    // 不在mapping里自动补充
    if (it.type === 'reference' || it.subtype === 'reference') {
      if (it.src) {
    ...
    } else if (it.type === 'object' || it.subtype === 'object') {
