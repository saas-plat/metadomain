  ...
  obj && Object.keys(obj).forEach(key => {
    ...
    if (typeof obj[key] === 'string') {
      ...
      fields = type === 'obj' || type === 'object' || type === 'reference' || type === 'array' ?
        ...
        undefined;
      defValue = other.defValue || defaultValue || DefValue || DefaultValue;
      src = src;
      rules = {
        type,
        ...other
...
const getFieldMapings = exports.getFieldMapings = (fields = [], defFields = []) => {
  return fields.reduce((maps, it) => {
    if (it.type === 'object') {
      const ret = getFieldMapings(it.fileds, defFields[it.key] || []);
...
const checkRequiredFieldMap = exports.checkRequiredFieldMap = (fields, mappings) => {
  ...
  fields.forEach(it => {
    ...
    if (it.type === 'reference') {
      ...
      const rfm = checkRequiredFieldMap(it.fileds, mappings[it.key]);
      defectFields.push({
        ...it,
...
const getKeyPaths = exports.getKeyPaths = (fields = [], pname = '') => {
  return (fields).reduce((paths, it) => {
    ...
    if (it.type === 'object') {
      return paths.concat(...getKeyPaths(it.fileds, it.key));
    } else if (it.type === 'array') {
      return paths.concat(...getKeyPaths(it.fileds || [], it.key));
    }
