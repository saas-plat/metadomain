...
const tableTypes = exports.tableTypes = ['string', 'object', 'array', 'number', 'bool', 'date', 'reference', 'buffer', 'mixed'];
...
const unset = exports.unset = (data, fields) => {
  ...
  _.keys(data).forEach(key => {
    ...
    if (!field) {
      ...
      if (field.type === 'array') {
        _.toArray(data[key]).forEach((it, i) => {
          unsetobj = {
            ...unsetobj,
            ..._.mapKeys(unset(it, field.fields), (v, subkey) => key + '.' + i + '.' + subkey)
          };
        })
      } else {
        unsetobj = {
          ...unsetobj,
          ..._.mapKeys(unset(data[key], field.fields), (v, subkey) => key + '.' + subkey)
        };
      }
    }
  });
...
const getRefrences = exports.getRefrences = (fields = [], versionFinder = name => name, pname = '') => {
  return fields.reduce((refs, it) => {
    const key = [pname, it.key].filter(it => it).join('.');
    if (it.type === 'reference') {
      return {
        ...refs,
        [key]: versionFinder(it.src)
      }
      ...
      return {
        ...refs,
        ...getRefrences(it.fields, versionFinder, it.key)
      }
    } else if (it.subtype === 'object') {
      return {
        ...refs,
