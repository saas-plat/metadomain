...
const MetaEntity = exports.MetaEntity = class MetaEntity extends Entity {
  ...
  async _executeAction(before, doing, after, params, opts = {}) {
    ...
    debug('%s(%s) fire %s...', this.constructor.key, this.id, after, floatEventData);
  }
  ...
  static createModel(BaseType, name, schame, rules, opts = {}) {
    ...
    const props = createObj({}, fields);
    const actions = _.pickBy(props, _.isFunction);
    const defaultProps = _.omitBy(props, _.isFunction);
    //debug('props', props);
    ...
    for (const method of _.keys(actions)) {
      if (method.startsWith('_')) {
        throw new Error(t('实体行为{{method}}不能以"_"开头', {
          method
        }));
      }
      if (_.hasIn(SpecificEntity.prototype, method)) {
        throw new Error(t('自定义行为{{method}}和系统行为冲突', {
          method
        }));
      }
      ...
      const before = new Inflectors(method).toGerund();
      const after = new Inflectors(method).toPast();
      SpecificEntity.prototype['_' + doing] = async function (...args) {
        return await fn(...args);
      }
      SpecificEntity.prototype[method] = async function (...args) {
        await this._executeAction(before, doing, after, ...args);
      }
      debug('carete custom action: %s ...', method);
    }
