...
const MetaEntity = exports.MetaEntity = class MetaEntity extends Entity {
  ...
  _merge(data, target) {
    this.constructor.schema.maptoModel(data, target || this);
    return target || this;
  }
  ...
  async validate(data, opts = {}) {
    ...
    for (let it of this.constructor.schema.fields) {
      ...
      if (typeof r.validator === 'function') {
        debug('execute validator...', r.validator);
        if (!await r.validator.call(this, r, val, data, opts, {
            repository: rep({
              ns: this._ns
            })
          })) {
          throw new Error(r.message || (it.key + t('验证失败')))
        }
      } else if (typeof r.validator === 'string' || _.isArray(r.validator)) {
        let fn;
        if (_.isArray(r.validator)) {
          if (r.validator.every(it => it.indexOf('return ') === -1)) {
            fn = r.validator.map((it, i) => i === r.validator.length - 1 ? 'return ' + it : it).join('\n');
          } else {
            fn = r.validator.join('\n');
          }
        } else {
          if (r.validator.indexOf('return ') === -1) {
            fn = 'return ' + r.validator;
          } else {
            fn = r.validator;
          }
        }
        debug('execute validator...', fn);
        if (!await new Function('rule, value, source, options, scope', fn)
          .call(this, r, val, data, opts, {
            repository: rep({
              ns: this._ns
            })
          })) {
  ...
  toJS() {
    ...
    this.constructor.schema.maptoDto(this, dto);
    return dto;
  }

  formJS(dto) {
    this.constructor.schema.maptoModel(dto, this);
  }
