...
const MetaEntity = exports.MetaEntity = class MetaEntity extends Entity {
  ...
  static create(BaseType, name, schame, rules, opts = {}) {
    ...
    const entityName = (ns ? (ns + '.') : '') + name;
    if (caches.has(entityName)) {
      const entityType = caches.get(entityName);
      if (entityType.version === version) {
        return entityType;
      }
    }
    debug('----- %s(%s)%s -----', name, BaseType.name, version);
    const {
      fields,
      conflicts,
      references,
    ...
    readonly(SpecificEntity, 'type', name);
    readonly(SpecificEntity, 'basetype', BaseType.name);
    readonly(SpecificEntity, 'basename', (ns ? (ns + '.') : '') + BaseType.name);
    readonly(SpecificEntity, 'name', entityName);
