...
const MetaEntity = exports.MetaEntity = class MetaEntity extends Entity {
  ...
  async _executeAction(before, doing, after, params, opts = {}) {
    debug('execute action %s->%s=>%s', before, doing, after);
    // before
    ...
    if (!isCustom) {
      debug('execute %s...', '_executeDefault');
  ...
  static createSchame(BaseType, schame) {
    ...
    const {
      defectFields,
      errFields
    } = checkRequiredFieldMap(defFields, mappings);
    if (errFields.length > 0) {
      throw new Error(t('{{entityName}}类型实体缺失必要字段{{fields}}', {
