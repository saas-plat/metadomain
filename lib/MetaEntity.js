...
const MetaEntity = exports.MetaEntity = class MetaEntity extends Entity {
  ...
  async _executeAction(before, doing, after, params, opts = {}) {
    debug('%s(%s) execute %s', this.constructor.key, this.id, doing, params);
    // before
    ...
    debug('%s(%s) fire %s...', this.constructor.key, this.id, after, changes);
  }

  async _executeDefault(params) {
    params = this._executeDefaulting ? await this._executeDefaulting(params) : params;
    //debug(33,params)
    const changes = filterValues(this._merge(params, this._createObj()));
    //  debug(44,changes)
    // 不需要计算差异需要保留所有的有用字段包括相同值的字段
    //const diffs = changes; //diff(changes, this.toJS());
    createMappingProps(changes, this.constructor.mappings);
    return this._executeDefaulted ? await this._executeDefaulted(changes) : changes;
  }
  ...
  async _executeHandler(method, data) {
    ...
    debug('%s(%s) handle %s...', this.constructor.key, this.id || '*REPLAY*', method, data);
    this._merge(data);
  ...
  static create(BaseType, name, schame, rules, opts = {}) {
    if (!name) {
    ...
    const handleAction = async (ns, action, data, ...datas) => {
      const nsActions = ns ? [
        ns + '.' + action, // 特定范围所有行为
    ...
    const SpecificEntity = class extends mix(MetaEntity, BaseType) {
      _init() {
      ...
      async onAction(action, data, ...objs) {
        ...
        return await fn(this._ns, action, data, this, ...objs);
      }
