...
const syskeys = ['eventsToEmit', 'newEvents', 'replaying', 'snapshotVersion', 'timestamp', 'version', '_events', '_eventsCount', '_maxListeners', '_type'];
...
const MetaEntity = exports.MetaEntity = class MetaEntity extends Entity {
  ...
  formatData(data) {
    ...
    for (let it of this.constructor.fields) {
      ...
      if (it.key in data && !_.isUndefined(data[it.key])) {
        if (_.isNull(data[it.key])) {
          // 支持null
          formatData[it.key] = null;
        } else {
          switch (it.type) {
          case 'string':
  ...
  async _executeAction(before, doing, after, params, opts = {}) {
    debug('%s(%s) execute %s->%s=>%s', this.constructor.name, this.id, before, doing, after);
    // before
    ...
    // 规则校验
    await this.checkData(this._merge(changes, this.toJS()), opts);
    // after 触发回溯事件
    ...
    debug('fire ' + after, changes);
  }

  async _executeDefault(params) {
    let changes = this._merge(params, this._createObj());
  ...
  async _executeHandler(method, data) {
    debug('%s(%s) handle %s...', this.constructor.name, this.id, method, data);
    this._merge(data);
  ...
  static create(BaseType, name, schame, rules, eventHandler = {}) {
    ...
    const SpecificEntity = class extends BaseType {
      ...
      async onAction(action, data, ...objs) {
        debug('%s(%s) on %s action...', this.constructor.name, this.id, action, ...objs)
