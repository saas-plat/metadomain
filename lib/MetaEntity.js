...
const MetaEntity = exports.MetaEntity = class MetaEntity extends Entity {
  ...
  async _executeAction(before, doing, after, params, opts = {}) {
    debug('%s(%s) execute %s->%s=>%s', this.constructor.name, this.id, before, doing, after, params);
    // before
    ...
    if (!isCustom) {
      ...
      changes = fn ? (await fn(changes) || changes) : changes;
    }
  ...
  //  debug(11,changes,diffs,this.toJS().updateBy)
