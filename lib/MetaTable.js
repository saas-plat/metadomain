...
const cache = exports.cache = new NodeCache({
  stdTTL: process.env.TABLE_TIMEOUT || 60 * 60, // 1h
  useClones: false,
});
cache.on("expired", function (key, value) {
  debug('%s table expired...', key);
  delete mongoose.modelSchemas[ukey];
  delete mongoose.models[ukey];
});
  ...
  obj && obj.forEach(item => {
    ...
    case 'array':
      ...
      case 'string':
        ...
        break;
      case 'object':
        itemtype = new mongoose.Schema(createDbSchema(item.fields, {
          autopopulate
        }), defOptions);
        break;
      case 'reference':
...
exports.MetaTable = class MetaTable {
  ...
  static create(BaseTable, name, meta, rules, opts = {}) {
    ...
    if (mongoose.modelNames().indexOf(ukey) > -1) {
      const model = mongoose.model(ukey);
