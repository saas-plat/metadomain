...
// ObjectId
// Array
const createDbSchema = async (obj, refVersionFinder) => {
  const schema = {};
  if (!obj){
    return schema;
  }
  for(const item of obj) {
    ...
    case 'object':
      // 这个是 mixed
      // schema[key] = {
      //   type: await createDbSchema(item.fields,refVersionFinder),
      //   default: undefined
      // };
      schema[key] = await createDbSchema(item.fields, refVersionFinder);
      break;
    case 'reference':
      schema[key] = {
        type: mongoose.Schema.Types.ObjectId,
        ref: refVersionFinder ? (await refVersionFinder(item.src)) : item.src,
      };
      break;
    case 'array':
      ...
      case 'object':
        ...
        subsche = new mongoose.Schema(await createDbSchema(item.fields, refVersionFinder), defOptions);
        break;
      case 'reference':
        subsche = {
          type: {
...
const getReferenceModel = async (refName, ns, getReferenceVersion, checkVersion = false) => {
  ...
  const ukey = refName + (refVersion ? '_' + refVersion : '');
  const model = cache.get(ukey);
  const collectionName = (ns ? (ns + '.') : '') + refName;
  if (collectionName !== model.collection.name) {
    debug('%s subclass...', ukey, collectionName);
...
exports.MetaTable = class MetaTable {
  ...
  static createSchema(BaseTable, meta, {
    ...
    const refVersionFinder = async (name) => {
      const refVersion = getReferenceVersion ? (await getReferenceVersion(name)) : null;
      if (checkVersion && !refVersion) {
        throw new Error(t('引用对象{{name}}无法获取版本号', {
          name
        }));
      }
      return name + (refVersion ? '_' + refVersion : '');
    }
    const sche = await createDbSchema(fields, refVersionFinder);
    const references = getRefrences(fields, refVersionFinder);
    //debug(sche._mid);
