  ...
  static create(BaseTable, name, meta, rules, opts = {}) {
    ...
    if (model) {
      ...
      if (collectionName !== model.collection.name) {
        ...
        model = model.__subclass(mongoose.connection, newSchema, collectionName);
      }
      return model;
    }
    ...
    const getReferenceModel = async refName => {
      ...
      return model.__subclass(mongoose.connection, schema, collectionName);
    }
    ...
    if (!query) {
      ...
      schema.static('getReferenceModel', (name) => {
        return getReferenceModel(name);
      });
    ...
    if (populateReferences) {
      ...
      schema.
      post('find', (docs) => populateReferencesHandler(references, populateService || getReferenceModel, ...docs)).
      post('findOne', (doc) => populateReferencesHandler(references, populateService || getReferenceModel, doc)).
      post('findOneAndUpdate', (doc) => populateReferencesHandler(references, populateService || getReferenceModel, doc));
    }
