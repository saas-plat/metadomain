...
exports.MetaTable = class MetaTable {
  ...
  static createSchema(BaseTable, name, meta, scope = {}) {
  ...
  static create(BaseTable, name, meta, scope = {}, rules) {
    const schema = MetaTable.createSchema(BaseTable, name, meta, scope);
    schema.static({
    ...
    const fireAction = async (action, doc) => {
      ...
      await ruleset.execute([...events, ...env.tables]);
    }
