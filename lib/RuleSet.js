...
const {modifiers} = require('nools/lib/compile/common');
...
const parseAction = (action, identifiers, defined, scope) => {
  ...
  try {
    console.log(new Function(params.join(","), action).toString())
    ...
    try {
      ...
      this.flow = nools.getFlow(name) || isDsl ? nools.compile(noolsSource.join('\n'), {
        ...
        for (let {
          ...
          parseRule(when, define, identifiers);
          // debug('when:%o', when);
          // debug('identifiers:%o', identifiers);
          // 结构别名的变量
          flow.rule(name, {
            scope: ruleScope
          }, when, parseAction(then.join('\n'), identifiers, define, ruleScope));
        }
      });
    } catch (err) {
