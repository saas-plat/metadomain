...
exports.Migration = class Migration {
  ...
  async up(fromVersion, toVersion) {
    ...
    for (const entityRep of this.migrationTypes) {
      ...
      await this._migrationEvents(entityRep, fromVersion, toVersion);
    }
  }

  async _migrationEvents(entityRep, fromVersion, toVersion) {
    ...
    while (eit) {
      ...
      const data = await this._migrateAction(actions, eit.id, fromVersion, toVersion, eit.method, eit.data);
      // 需要更新
  ...
  async _migrateAction(actions, id, fromVersion, toVersion, method, data) {
    if (!this._onAction) {
      return data;
    }
