...
exports.Migration = class Migration {
  ...
  async up(scope) {
    ...
    for (const entityRep of this.migrationTypes) {
      ...
      await this._migrationEvents(entityRep, scope);
    }
  }

  async _migrationEvents(entityRep, scope) {
    ...
    while (eit) {
      ...
      const data = await this._migrateAction(actions, eit.id, scope, eit.method, eit.data);
      // 需要更新
  ...
  async _migrateAction(actions, id, scope, method, data) {
    if (!this._onAction) {
      return data;
    }
