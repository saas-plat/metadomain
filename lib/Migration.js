...
exports.Migration = class Migration {
  ...
  async _migrationEvents(entityRep) {
    ...
    const actions = [entityType.name + '.migrate', , entityType.schema.type + '.migrate', , 'migrate'].concat(scopeActions);
    let eit = await cur.next();
    while (eit) {
      // 执行升级
      debug('execute %s migration rules...', eit.id);
      const data = await this._migrateAction(actions, eit);
      // 需要更新
  ...
  async _migrateAction(actions, eit) {
    if (!this._onAction) {
      return eit.data;
    }
    debug('execute %s migration rules...', eit.id);
    const old = _.cloneDeep(eit.data);
    const evt = new Event(eit.method, _.cloneDeepWith(old, value => {
      // TODO 类型转换
      return value;
    }));
    debug(evt, old)
    await this._onAction([
      ...actions.map(action => (new Action(action, eit.data, {
        event: eit.method,
        id: eit.id
      }))),
      evt
