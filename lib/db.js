...
const connectEntity = async (options) => {
  if (mongodbConnected) {
    debug('mongodb has connected!')
    return connctdb;
  }
  options = options || {};
  mongodbConnected = true;
  var url = 'mongodb://' + ((options.user || process.env.MONGO_USER) ? (encodeURIComponent(options.user || process.env.MONGO_USER) + ':' + encodeURIComponent(options.password || process.env.MONGO_PASSWORD) + '@') : '') +
    (options.url || process.env.MONGO_URL || (process.env.MONGO_PORT_27017_TCP_ADDR ? (process.env.MONGO_PORT_27017_TCP_ADDR + ':27017/sourced') : '') || "localhost/sourced");
