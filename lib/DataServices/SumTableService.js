...
exports.SumTableService = class SumTableService extends BaseService {
  ...
  async onSaved(data) {
    ...
    const {
      ...
      detailMapping = createDefaultMappings(data[detailKey], [], detailKey + '_'),
    } = this.options;
    ...
    if (detailKey in data) {
      ...
      }, this.Model.schema.get('mappings'))));
      // 更新或添加明细
      for (const it of details) {
        ...
        //debug(it, detail)
        await this.Model.upsert(parseUserData(detail, this.Model.schema.get('mappings')));
      }
    }
    // 更新表头需要把所有明细都更新
