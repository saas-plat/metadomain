...
const repWapper = (repository, scope) => {
  ...
  return {
    ...
    get: async (...ids) => {
      ...
      const entities = (await Promise.all(ids.map(async id => commits.find(it => it.id === id) || await findRep(repository, id)))).filter(it => it);
...
exports.Repository = {
  ...
  create: (EntityType, scope) => {
    ...
    const ret = repWapper(repository, scope);
