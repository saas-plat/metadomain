  ...
  return {
    ...
    init: async()=>{
      for (const index of repository.indices) {
        await repository.snapshots.ensureIndex(index);
        await repository.events.ensureIndex(index);
      }
      await repository.events.ensureIndex({
        id: 1,
        version: 1
      });
      await repository.snapshots.ensureIndex({
        id: 1,
        version: 1
      });
      await repository.snapshots.ensureIndex('snapshotVersion');
    }
  };
}
