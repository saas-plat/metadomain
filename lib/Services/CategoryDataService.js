...
const CategoryDataService = exports.CategoryDataService = class CategoryDataService extends BaseService {
  ...
  async _reduceCheckChilds(items) {
    if (!items || items.length<=0){
      return;
    }
    for (const it of items) {
      if (it.details.length > 0) {
        throw new BizError(t('分类至少还有{{count}}条数据不能直接删除!', {
          count: it.details.length
        }))
      }
    }
    const ids = Array.from(new Set(items.reduce((arr, it) => it.childs ? arr.concat(it.childs.map(it => it.id)) : arr, [])));
    const childs = await this.levelService.repository.get(...ids);
    await this._reduceCheckChilds(childs);
  }

  async deleteCategory(...ids) {
    const results = [].concat(await this.levelService.delete(...ids));
    debug('delete category ...',results.length)
    await this._reduceCheckChilds(results);
  ...
  async saveData(...datas) {
    ...
    for (const categoryId of distincts) {
      ...
      debug('save category(%s) details...', categoryId);
  ...
  async deleteData(...ids) {
    const details = [].concat(await this.delete(...ids));
    ...
    return ids.length === 1 ? details[0] : details;
  }
}
