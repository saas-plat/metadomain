...
const CategoryDataService = exports.CategoryDataService = class CategoryDataService extends BaseService {
  ...
  async saveData(...datas) {
    const distincts = datas.reduce((cids, it) => {
      debug(it,this.mapping);
      const categoryId = it[this.mapping.category].id;
    ...
    for (const categoryId of distincts) {
      const ds = datas.filter(it => it[this.mapping.category].id === categoryId);
      ...
      if (!category){
        throw new BizError(t('数据所属分类({{categoryId}})未找到!',{categoryId}))
      }
      const exists = category[this.mapping.details].map(id =>  id);
      await this.levelService.save({
        id: categoryId,
        [this.mapping.details]: exists.concat(details.filter(it=>exists.indexOf(it.id)===-1).map(id => ({
          id
        })))
      });
    }
