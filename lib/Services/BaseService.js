...
const BaseService = exports.BaseService = class BaseService {
  ...
  get repository() {
    ...
    if (!this._repository) {
      throw new Error(t('主实体{{name}}仓库无法加载！', {
  ...
  async _loadReferences(data, paths, index = 0, prePath = '') {
    ...
    if (refData) {
      // 加载引用实体仓库
      const entityName = this.entityType.schema.references[paths.join('[]')];
  ...
  async _loadData(data, entityType = this.entityType) {
    const bdata = parseBigData(data, entityType.schema.mappings);
    const references = entityType.schema.references;
    ...
    //debug('load mappings', entityType.schema.mappings, bdata)
    const udata = parseUserData(bdata, entityType.schema.mappings);
    return udata;
  }
