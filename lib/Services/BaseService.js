...
const BaseService = exports.BaseService = class BaseService {
  constructor(entity, context = {}, findRepository = createRep(entity)) {
    if (!entity) {
  ...
  async _handleReference(refData, refRepository) {
    ...
    if (!entity && refData) {
      if (refData.id) {
        ...
        const Service = require('./index')[refRepository.entityType.prototype.name.replace('Data', 'Service')];
        if (!Service) {
          throw new Error(t('{{name}}实体服务查找失败！', refRepository.entityType));
        }
        const service = new Service(refRepository.entityType, this.context, this.findRepository);
  ...
  async _loadData(data, entityType = this.entityType) {
    const udata = parseData(data, entityType.mappings);
    debug('load mappings', entityType.mappings, udata)
    ...
    for (const path of Object.keys(references)) {
      ...
      await this._loadReferences(udata, arrSplits);
    }
    return udata;
  }
  ...
  async save(...datas) {
    ...
    for (const data of datas) {
      ...
      let entity = params.id ? await this.repository.get(params.id) : null;
      if (!entity) {
        //  debug(params)
  ...
  async customAction(action, ...datas) {
    ...
    for (const data of datas) {
      ...
      if (!entity) {
        entity = await this.entityType.create(params.id);
      }
  ...
  async saveStatus(...datas) {
    ...
    for (const data of datas) {
      // 解析大数组
      const params = await this._loadData(data);
      const entity = await this.repository.get(params.id);
      if (!entity) {
